# Script for sanity checking of data 
import sqlite3
import time
import zlib
...
for (message_id, message) in list(messages.items()):
    sender = message[1] # Pulling out the sender ID 
    sendcounts[sender] = sendcounts.get(sender,0) + 1 # Basic dictionary histogram for the people  
    pieces = senders[sender].split("@")
    if len(pieces) != 2 : continue
    dns = pieces[1]
    sendorgs[dns] = sendorgs.get(dns,0) + 1 # Basic dictionary histogram for the domains

print('')
print('Top',howmany,'Email list participants')

x = sorted(sendcounts, key=sendcounts.get, reverse=True) # sorted(iterable, key, reverse)
for k in x[:howmany]:
